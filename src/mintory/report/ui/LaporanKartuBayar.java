...
//           }
    ...
    private void btnMuatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnMuatActionPerformed
        // TODO add your handling code here:
        listClosingBulan = Main.getTransaksiService().closingBulananFirstRecord(new Integer(txtLambung.getText()));
        try{
        JasperPrint getReport = new JasperPrint();
//        listClosingSaldoAwal = Main.getTransaksiService().closingBulananRecordForSaldoAwalAll(dateAwal.getDate(), Main.getTransaksiService().findByNoLambung(new Integer(txtLambung.getText()), dateAwal.getDate()).get(0).getSetor_map().getIdKomposisi());
        closingBulanan closingSaldoAwal = Main.getTransaksiService().findClosingSaldoAwalByRefNoLambung(new Integer(txtLambung.getText()), dateAwal.getDate());
        currentMonth = new DateTime(dateAwal.getDate()).getMonthOfYear();
        firstSaldoAwalMonth = new DateTime(listClosingBulan.get(0).getPeriodeBulan()).getMonthOfYear();
        if(currentMonth == firstSaldoAwalMonth) {
            isFirstClosing = true;
        } else {
            isFirstClosing = false;
        }
        if (closingSaldoAwal !=null) {
            getReport = Main.getReportService().getKartuPembayaranReport(new Integer(txtLambung.getText()), dateAwal.getDate(), dateAkhir.getDate(), isClosedFor.CLOSING_SALDO_AWAL, new DateTime(dateAwal.getDate()).getMonthOfYear(), "NOT NULL" , OsUtils.getOsName());
        } else {
            if (isFirstClosing) {
                getReport = Main.getReportService().getKartuPembayaranReport(new Integer(txtLambung.getText()), dateAwal.getDate(), dateAkhir.getDate(), isClosedFor.CLOSING_SALDO_BULAN_LALU, new DateTime(dateAwal.getDate()).getMonthOfYear(), "NOT NULL" , OsUtils.getOsName());
            } else {
                getReport = Main.getReportService().getKartuPembayaranReport(new Integer(txtLambung.getText()), dateAwal.getDate(), dateAkhir.getDate(), isClosedFor.CLOSING_SALDO_BULAN_LALU, new DateTime(dateAwal.getDate()).minusMonths(1).getMonthOfYear(), "NULL" , OsUtils.getOsName());
            }
...
//            dateAkhir.setDate(new DateTime(st.getIdClosing().getPeriodeBulan().getTime()).withDayOfMonth(new DateTime(st.getIdClosing().getPeriodeBulan()).dayOfMonth().getMaximumValue()).plusMonths(1).toDate());
        ...
        pnlReport.updateUI();
        }catch(IndexOutOfBoundsException ioBounds) {
            JOptionPane.showMessageDialog(this, "Tidak Ada Data Kartu No.Lambung "+txtLambung.getText()+" Untuk Ditampilkan Pada Bulan "+new DateTime(dateAwal.getDate()).monthOfYear().getAsText(),"Error", JOptionPane.ERROR_MESSAGE);
        }
...
//            updateUI();
    ...
    private void dateAwalPropertyChange(java.beans.PropertyChangeEvent evt) {//GEN-FIRST:event_dateAwalPropertyChange
        // TODO add your handling code here:
        dateAkhir.setDate(new DateTime(dateAwal.getDate()).withDayOfMonth(new DateTime(dateAwal.getDate()).dayOfMonth().getMaximumValue()).toDate());
