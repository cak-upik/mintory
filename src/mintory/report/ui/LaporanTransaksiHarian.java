...
public class LaporanTransaksiHarian extends javax.swing.JInternalFrame {
    ...
    public LaporanTransaksiHarian() {
        ...
        int maxDay = new DateTime(sys.getTglKerja()).dayOfMonth().getMaximumValue();
        int currentDay = new DateTime(sys.getTglKerja()).getDayOfMonth();
        if (currentDay == maxDay || currentDay == maxDay - 1) {
            listKomposisi = Main.getSistemService().komposisiBonusRecord();
            dateSPO.setMinSelectableDate(new DateTime(sys.getTglKerja()).withDayOfMonth(new DateTime(sys.getTglKerja()).dayOfMonth().getMaximumValue()).minusDays(1).toDate());
            dateSPO.setMaxSelectableDate(new DateTime(sys.getTglKerja()).withDayOfMonth(new DateTime(sys.getTglKerja()).dayOfMonth().getMaximumValue()).toDate());
            dateSPO.setDate(sys.getTglKerja());
        } else {
            listKomposisi = Main.getSistemService().komposisiRecord();
            dateSPO.setMaxSelectableDate(new DateTime(sys.getTglKerja()).withDayOfMonth(new DateTime(sys.getTglKerja()).dayOfMonth().getMaximumValue()).minusDays(2).toDate());
            dateSPO.setMinSelectableDate(new DateTime(sys.getTglKerja()).withMonthOfYear(new DateTime(sys.getTglKerja()).dayOfMonth().getMinimumValue()).toDate());
            dateSPO.setDate(sys.getTglKerja());
        }
    ...
    private void btnMuatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnMuatActionPerformed
        // TODO add your handling code here:
        JasperPrint getReport = Main.getReportService().getDailyTransactionReport(dateSPO.getDate(), listKomposisi.get(cmbKomposisi.getSelectedIndex()), OsUtils.getOsName());
